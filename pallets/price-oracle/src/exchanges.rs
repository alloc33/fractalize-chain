...
impl<C: ChainInterface, P: DexProtocol<C>> Exchange<C, P> {
	...
	pub fn fetch_price(&self, pair: TokenPair, timeout_ms: u64, min_price: u64, max_price: u64) -> Result<(u64, u64), http::Error> {
		...
		if price_micro < min_price || price_micro > max_price {
			log::error!("Price out of bounds: {} (min: {}, max: {})", price_micro, min_price, max_price);
...
pub trait ExchangeInterface {
	fn fetch_price(&self, pair: TokenPair, timeout_ms: u64, min_price: u64, max_price: u64) -> Result<(u64, u64), http::Error>;
